...
class StorageManager {
  ...
  private async getOrCreateEncryptionKey(): Promise<string> {
    ...
    const existing = this.asRecord(
      await this.localGet(ENCRYPTION_KEY_STORAGE_KEY),
  ...
  private async getDecryptionKeys(): Promise<string[]> {
    const runtimeLegacyKey = chrome?.runtime?.id
  ...
  async getAll(): Promise<StorageData> {
    try {
      ...
      if (!this.migrationChecked) {
        const legacyEncryptedResult = this.asRecord(
          await this.localGet([...LEGACY_STORAGE_KEYS]),
        );
        for (const key of LEGACY_STORAGE_KEYS) {
          ...
          if (typeof candidate === "string" && candidate.length > 0) {
            ...
            if (decryptedLegacy) {
              const normalizedLegacy =
                this.normalizeStorageData(decryptedLegacy);
          ...
          if (candidate && typeof candidate === "object") {
            const normalizedLegacy = this.normalizeStorageData(
              candidate as Partial<StorageData>,
            );
            await this.saveAll(normalizedLegacy);
            this.migrationChecked = true;
            return normalizedLegacy;
  ...
  async saveAll(data: StorageData): Promise<void> {
    ...
    if (normalized.user.settings.syncEnabled) {
      try {
        ...
        console.warn(
          "Unable to sync encrypted storage to chrome.storage.sync:",
          error,
        );
      }
    }
  ...
  async getSnippets(): Promise<Snippet[]> {
    ...
    const localSnippets = this.mergeSnippets(
      await this.getLegacySnippetCandidates(),
      localPrimarySnippets,
    );
  ...
  async saveSnippet(snippet: Snippet): Promise<void> {
    await this.enqueueWrite(async () => {
      const data = await this.getAll();
      const snippets = this.mergeSnippets(
        this.sanitizeSnippets(data.snippets),
        await this.getLegacySnippetCandidates(),
      );
      ...
      if (!normalized) {
        throw new Error(
          "Snippet is invalid. Ensure title, shortcut, and content are valid.",
        );
      }
      ...
      try {
        ...
        console.warn(
          "Unable to sync encrypted storage after saveSnippet:",
          error,
        );
      }
    });
  ...
  async deleteSnippet(id: string): Promise<void> {
    await this.enqueueWrite(async () => {
      const data = await this.getAll();
      const snippets = this.sanitizeSnippets(data.snippets).filter(
        (s) => s.id !== id,
      );
      ...
      try {
        ...
        console.warn(
          "Unable to sync encrypted storage after deleteSnippet:",
          error,
        );
      }
    });
  ...
  private getDefaultSnippets(): Snippet[] {
    ...
    return [
      ...
      },
      {
        id: "default-gm",
        shortcut: "/gm",
        content: "yourname@gmail.com",
        title: "Gmail Address",
        category: "Personal",
        tags: ["gmail", "email", "contact"],
        usageCount: 0,
        createdAt: now,
        updatedAt: now,
        isActive: true,
      },
      {
        id: "default-li",
        shortcut: "/li",
        content: "Hi [Name],\n\nI hope you're doing well. I came across your profile and was really impressed by your background in [Industry/Role]. I'd love to connect and keep in touch!\n\nBest regards,\n[Your Name]",
      ...
      },
      {
        id: "default-gh",
        shortcut: "/gh",
        content: "https://github.com/GoldLion123RP/typewise-extension",
      ...
      },
      {
        id: "default-add1",
        shortcut: "/add1",
        content: "123 Innovation Way, Tech District, San Francisco, CA 94105",
        title: "Home Address",
        category: "Personal",
      ...
      },
      {
        id: "default-bio",
        shortcut: "/bio",
        content: "Rahul Pal is a software engineer specializing in frontend technologies and browser extensions. With a passion for building clean, user-friendly, and secure tools, he works with TypeScript, React, and modern web APIs to enhance productivity and simplify workflows.",
        title: "Professional Bio",
        category: "Personal",
        tags: ["bio", "profile", "about-me"],
        usageCount: 0,
        createdAt: now,
        updatedAt: now,
        isActive: true,
  ...
  private normalizeSnippet(value: unknown, index: number): Snippet | null {
    ...
    const rawShortcut = this.pickString(
      item.shortcut,
      item.trigger,
      item.abbr,
      item.key,
    ...
    const rawContent = this.pickString(
      item.content,
      item.text,
      item.value,
      item.body,
    ...
    const title = (
      this.pickString(item.title, item.name, item.label, item.shortcut) ||
      `Snippet ${index + 1}`
    ).slice(0, MAX_TITLE_LENGTH);

    return {
      id:
        this.pickString(item.id, item._id, item.uuid) ||
        `snippet-${Date.now()}-${index}`,
      title,
      shortcut,
      content,
      category:
        this.pickString(item.category, item.group, item.type) || "General",
      tags,
      usageCount:
  ...
  private normalizeStorageData(
    value: Partial<StorageData> | undefined,
  ): StorageData {
    const defaults = this.getDefaultData();
    ...
    return {
      ...
      ...value,
      snippets:
        value && "snippets" in value ? normalizedSnippets : defaults.snippets,
      user: {
        ...defaults.user,
        ...value?.user,
      ...
      encryptionEnabled:
        typeof value?.encryptionEnabled === "boolean"
