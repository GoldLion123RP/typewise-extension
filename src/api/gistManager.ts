...
export class GistManager {
  ...
  async authenticate(): Promise<string> {
    if (!this.CLIENT_ID || this.CLIENT_ID === "your_client_id_here") {
      throw new Error(
        "GitHub client ID is not configured. Update GITHUB_CLIENT_ID in your environment.",
      );
    }
    ...
    if (!deviceCodeResponse.ok) {
      throw new Error(
        `GitHub device flow error: ${deviceCodeResponse.statusText}`,
      );
    }
    ...
    if (!deviceCodeData.device_code || !deviceCodeData.verification_uri) {
      throw new Error(
        "GitHub device authorization did not return a valid verification URL.",
      );
    }
    ...
    while (Date.now() < expirationAt) {
      ...
      if (!tokenResponse.ok) {
        throw new Error(
          `GitHub token polling error: ${tokenResponse.statusText}`,
        );
      }
      ...
      if (tokenData.error === "slow_down") {
        await new Promise((resolve) =>
          window.setTimeout(resolve, pollInterval + 5000),
        );
        continue;
      }
      ...
      if (tokenData.error) {
        throw new Error(
          tokenData.error_description ||
            `GitHub authentication failed: ${tokenData.error}`,
        );
      }
    ...
    throw new Error(
      "GitHub device authorization expired before it was approved.",
    );
  }
  ...
  async fetchGitHubUsername(token: string): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to verify GitHub account: ${response.statusText}`,
      );
    }
  ...
  async createOrUpdateGist(
    ...
    const gistData = {
      description:
        "TypeWise Snippets - Last updated: " + new Date().toISOString(),
      public: false,
      files: {
        [this.GIST_FILENAME]: {
