...
class PopupManager {
  ...
  async init() {
    ...
    try {
      ...
      this.renderSnippets();
      this.showToast(
        "Recovered from a data error. Try opening settings once.",
        "warning",
      );
    }
  }
  ...
  renderSnippets(filter: string = "") {
    ...
    const filteredSnippets = this.snippets.filter(
      (s) =>
  ...
  attachEventListeners() {
    ...
    // Internal snippet expansion listener for Popup page inputs/textareas
    ...
    if (modal) {
      ...
      if (snippet) {
        ...
        (document.getElementById("snippetShortcut") as HTMLInputElement).value =
          snippet.shortcut;
        (
          document.getElementById("snippetContent") as HTMLTextAreaElement
        ).value = snippet.content;
        (
          document.getElementById("snippetCategory") as HTMLSelectElement
        ).value = snippet.category || "General";
  ...
  async saveSnippet() {
    const title = (
      document.getElementById("snippetTitle") as HTMLInputElement
    ).value.trim();
    const shortcut = (
      document.getElementById("snippetShortcut") as HTMLInputElement
    ).value.trim();
    const content = (
      document.getElementById("snippetContent") as HTMLTextAreaElement
    ).value.trim();
    ...
    const tags = (
      document.getElementById("snippetTags") as HTMLInputElement
    ).value
      .split(",")
      .map((t) => t.trim())
      .filter((t) => t.length > 0);
    ...
    const snippet: Snippet = {
      ...
      usageCount: this.currentEditId
        ? this.snippets.find((s) => s.id === this.currentEditId)?.usageCount ||
          0
        : 0,
  ...
  async updateStats() {
    ...
    if (syncButton) {
      syncButton.title = user.githubToken
        ? "Sync with GitHub"
        : "Connect GitHub in Settings to sync";
  ...
  async loadSettings() {
    try {
      const user = await storage.getUser();
      if (user?.settings) {
        this.triggerKey = user.settings.triggerKey || "/";
        this.caseSensitive = Boolean(user.settings.caseSensitive);
        this.playSounds = user.settings.playSounds !== false;
        this.soundVolume =
          typeof user.settings.soundVolume === "number"
            ? user.settings.soundVolume
            : 30;
      }
    } catch (e) {
      console.error("Failed to load settings in popup", e);
    }
  }
